...
Let's go ahead and execute a basic description of the student dataset (see Table~\ref{tab:basicfacts}):
\begin{table}
\centering
\begin{tabular}{l|r}
Item & Value \\\hline
...
Due to the nature of the data, in ``Preparing the Data'' section, the features will be coded via \href{_dummy variables_}{https://www.moresteam.com/whitepapers/download/dummy-variables.pdf}. As described by \cite{Hastie_2009}, this method consists in representing a K-Level qualitative variable by a vector of K binary variables, where only one of which is ``on'' at a time. There is no missing data on this dataset and no presence of ``real'' outliers.
